...
app.get('/search', function(req, res) {
	var search = req.query.searchTerm;//gets the .val thing to search with
	//part of #6 - 1. search through dummydata for searchTerm
	if(search in data.programming) {
		res.send(data.programming[search].desc);
	} else if (search in data["search engines"]) {
		res.send(data["search engines"][search].desc);
	} else {
		res.send("No.");
	}
	//part of #6 - 2. send the result to the client 
	//res.send(programming.key.desc);

	res.send("test");
}) //#2 go to localhost:3000/search to test (see "test" in that window)
